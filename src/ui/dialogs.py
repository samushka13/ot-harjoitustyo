...
def show_invalid_username_dialog():
    return messagebox.showinfo('Invalid Username','Username should be 3 or more characters long.')

def show_login_error_dialog():
    return messagebox.showinfo('Login Error',"Username and password don't match.")

def show_login_success_dialog(username: str):
    return messagebox.showinfo('Logged in!',f'Welcome back, {username}!')

def show_username_created_dialog(username: str):
    return messagebox.showinfo('Username Created',f'Nice to meet you, {username}!')

def show_users_dialog():
    if len(get_sorted_users()) > 0:
        dialog = messagebox.showinfo('Users',f'List of users on this device: \n\n{get_sorted_users()}')
    else:
        dialog = messagebox.showinfo('Users','There are currently no users on this device.')
    return dialog
